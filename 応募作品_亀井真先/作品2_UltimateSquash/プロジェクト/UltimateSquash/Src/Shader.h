...
// MPR用のコンスタントバッファーのアプリ側構造体		//--2022.01.28.2300
struct CONSTANT_BUFFER_MPR
{
	VECTOR2    ViewPort;
	VECTOR2    Info;
};
...
public:
	...
	// Fbxモデル　スタティツクメッシュ用のシェーダー
	// （トゥーンシェーディング）
	ID3D11VertexShader*     m_pFbxStaticMeshToon_VS_TOON1;
	ID3D11PixelShader*      m_pFbxStaticMeshToon_PS_TOON1;
	ID3D11VertexShader*     m_pFbxStaticMeshToon_VS_TOON2;
	ID3D11PixelShader*      m_pFbxStaticMeshToon_PS_TOON2;

	// Fbxモデル　スキンメッシュ用のシェーダー
	// （トゥーンシェーディング）
	ID3D11VertexShader*     m_pFbxSkinMeshToon_VS_TOON1;
	ID3D11PixelShader*      m_pFbxSkinMeshToon_PS_TOON1;
	ID3D11VertexShader*     m_pFbxSkinMeshToon_VS_TOON2;
	ID3D11PixelShader*      m_pFbxSkinMeshToon_PS_TOON2;

	// Fbxモデル　スタティツクメッシュ用のシェーダー		//--2022.02.2.7:30
// （残像シェーディング）
	ID3D11VertexShader*     m_pFbxStaticMeshAfter_VS_AFTER1;
	ID3D11PixelShader*      m_pFbxStaticMeshAfter_PS_AFTER1;
	ID3D11VertexShader*     m_pFbxStaticMeshAfter_VS_AFTER2;
	ID3D11PixelShader*      m_pFbxStaticMeshAfter_PS_AFTER2;

	// Fbxモデル　スキンメッシュ用のシェーダー		//--2022.02.2.7:30
	// （残像シェーディング）
	ID3D11VertexShader*     m_pFbxSkinMeshAfter_VS_AFTER1;
	ID3D11PixelShader*      m_pFbxSkinMeshAfter_PS_AFTER1;
	ID3D11VertexShader*     m_pFbxSkinMeshAfter_VS_AFTER2;
	ID3D11PixelShader*      m_pFbxSkinMeshAfter_PS_AFTER2;


	// MPR	//--2022.01.28.2300
	ID3D11InputLayout*      m_pMPR_VertexLayout;
	ID3D11VertexShader*     m_pMPR_VS;
	ID3D11PixelShader*      m_pMPR_PS;
	...
	// コンスタントバッファー　MPR用	//--2022.01.28.2300
	ID3D11Buffer*           m_pConstantBufferMPR;
...
public:
	...
	HRESULT InitShaderFbxToon();	//--2022.02.2.7:30
	HRESULT InitShaderFbxAfter();
	HRESULT InitShaderMPR();	//--2022.01.28.2300
