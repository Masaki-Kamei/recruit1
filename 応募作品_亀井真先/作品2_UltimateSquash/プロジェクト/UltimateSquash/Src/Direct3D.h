...
struct AFTERSTATUS			//--2022.02.2.7:30	残像のステータス		
{
	MATRIX4X4  mWorldOld[4];
	int		   SaveFrame[4];
	int		   SaveFrameNo[4];
	AFTERSTATUS()
	{
...
public:
	...
	ID3D11RasterizerState*    m_pRStateLB;  // 左回りの裏側を表面とする   // トゥーンレンダリングで使用  // -- 2020.2.15
	...
	ID3D11DepthStencilView*   m_pTarget_DSTexDSV; // 現在のレンダーターゲット   // -- 2019.4.19
													  ...
													  // トゥーンテクスチャ
	ID3D11ShaderResourceView* m_pToonTexture;             //--2022.01.28.2300
