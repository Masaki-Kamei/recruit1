...
public:
	...
	MATRIX4X4			m_mWorldOld;	// 移動マトリックス（一つ前）
	MATRIX4X4			m_mWorld;		// 移動マトリックス
	MATRIX4X4			m_mWorldInv;	// 移動マトリックスの逆マトリックス
...
public:
	...
	BOOL	AddFbxLoad( const TCHAR*, const VECTOR3&);
	BOOL	AddFbxLoad(const CFbxMesh* pFbxMesh, const VECTOR3& vOffset);			// -- 2020.12.15
	...
	int     isCollisionSphere(const VECTOR3& vNowIn, const VECTOR3& vOldIn, const float& fRadius, VECTOR3& vHit, VECTOR3& vNormal);		// -- 2020.12.14
	...
	int     CheckWallMove(MATRIX4X4& mWorld, const MATRIX4X4& mWorldOld, float fRadius);
	...
	int     CheckCollisionMove(MATRIX4X4& mWorld, const MATRIX4X4& mWorldOld, float fRadius);
	...
	MATRIX4X4	GetWorldInv() { return m_mWorldInv; }
