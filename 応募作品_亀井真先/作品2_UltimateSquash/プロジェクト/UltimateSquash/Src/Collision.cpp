...
//	作成する
//	複数のメッシュオブジェクトを読み込むと、メッシュ接触判定用配列に
//	追加していくことができる
//
//  const CFbxMesh* pFbxMesh
...
BOOL CCollision::AddFbxLoad(const CFbxMesh* pFbxMesh, const VECTOR3& vOffset)
{
	...
	for (i = 0; i < pFbxMesh->m_dwMeshNum; i++)
	{
		...
		for (j = 0; j < pFbxMesh->m_pMeshArray[i].m_dwVerticesNum; j++, nNumvert++)
		{
			...
			{
				m_ColArray[n].pVert[nNumvert] = pFbxMesh->m_pMeshArray[i].m_vStaticVerticesNormal[j].Pos + vOffset;
			}
			else {
				m_ColArray[n].pVert[nNumvert] = pFbxMesh->m_pMeshArray[i].m_vSkinVerticesNormal[j].Pos + vOffset;
			}
...
void CCollision::MakeChkColMesh( const int& nNum, const VECTOR3& vMin, const VECTOR3& vMax)
{
	...
	for (i = 0; i < m_ColArray[n].nNumFace; i++)
	{
		...
		fx = fabs(m_ColArray[n].pFace[i].AABB.m_vMax.x - m_ColArray[n].pFace[i].AABB.m_vMin.x);
		fy = fabs(m_ColArray[n].pFace[i].AABB.m_vMax.y - m_ColArray[n].pFace[i].AABB.m_vMin.y);
		fz = fabs(m_ColArray[n].pFace[i].AABB.m_vMax.z - m_ColArray[n].pFace[i].AABB.m_vMin.z);
							...
							if (CheckLay(vVert, vNow, vOld, vFaceNorm, fNowDist, fOldDist, fLayDist, vInsPt) == 1)
							{
								...
								if (fLenMin > len)   // より近い交点を探す
								{
									...
									if (m_bMoveFlag)  // コリジョンマップが移動しているかどうか
									{
										...
										vNormal = GetPositionVector(XMMatrixTranslation(vFaceNorm.x, vFaceNorm.y, vFaceNorm.z) * m_mWorld);
							...
							if (CheckLay(vVert, vNow - vNormalRadius, vOld - vNormalRadius, vFaceNorm, fNowDist, fOldDist, fLayDist, vInsPt) == 1)
							{
								...
								if (fLenMin > len)   // より近い交点を探す
								{
									...
									if (m_bMoveFlag)  // コリジョンマップが移動しているかどうか
									{
										...
										vNormal = GetPositionVector(XMMatrixTranslationFromVector(vFaceNorm) * m_mWorld);
										vNormal = normalize(vNormal);
									}
									else {
...
int CCollision::isCollisionMove(MATRIX4X4& mWorldIn, const MATRIX4X4& mWorldOldIn, float fRadius)
{
	...
	// 接触判定とスリスリ動かす制御を行う
	nRet = CheckCollisionMove(mWorld, mWorldOld, fRadius);
	if ( nRet != 0)   // 接触して移動したときは、2度目の接触判定を行う
	{
		CheckCollisionMove(mWorld, mWorldOld, fRadius);
	}
...
//		接触していないとき	０
//-----------------------------------------------------------------------------
int CCollision::CheckCollisionMove(MATRIX4X4& mWorld, const MATRIX4X4& mWorldOld, float fRadius)
{
	int      nRet = 0;
	int      n, i;
	int      x, y, z, m;
...
int CCollision::isCollisionMoveGravity(MATRIX4X4& mWorldIn, const MATRIX4X4& mWorldOldIn, float fRadius)
{
	...
	nRetWall = CheckWallMove(mWorld, mWorldOld, fRadius);

	if (nRetWall != 0)   // 接触して移動したときは、2度目の接触判定を行う
	{
		CheckWallMove(mWorld, mWorldOld, fRadius);
	}
...
//		接触していないとき	０
//-----------------------------------------------------------------------------
int CCollision::CheckWallMove(MATRIX4X4& mWorld, const MATRIX4X4& mWorldOld, float fRadius)
{
	int      nRet = 0;
	int      n, i;
	int      x, y, z, m;
