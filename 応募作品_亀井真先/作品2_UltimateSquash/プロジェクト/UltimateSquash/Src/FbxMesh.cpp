...
CFbxMesh::CFbxMesh(CFbxMeshCtrl* pFbxMeshCtrl)
{
	...
	m_fHeightMax = 0.01f;                // ディスプレースメントマッピングの高さ    
	m_iMaxDevide = 64;                   // ディスプレースメントマッピングの分割数   
	m_vDiffuse = VECTOR4(1, 1, 1, 1);    // ディフューズ色                       
}
...
void CFbxMesh::DestroyD3D()
{
	...
	{
		m_pFbxMeshCtrl->DeleteTextureList(m_pTextureArray[i]);                 
	...
	SAFE_DELETE_ARRAY(m_pMaterialDiffuseArray);      
	SAFE_DELETE_ARRAY(m_pMaterialSpecularArray);    
...
BOOL CFbxMesh::Load(const TCHAR* FName)
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		m_pMaterialDiffuseArray = new VECTOR4[m_dwTextureNum];            // マテリアルディフューズカラー配列    
		m_pMaterialSpecularArray = new VECTOR4[m_dwTextureNum];           // マテリアルスペキュラ－カラー配列    
		...
		m_pTextureHeightArray = new ID3D11ShaderResourceView*[m_dwTextureNum];  // テクスチャーHeight配列を生成する  
		...
		for (mi = 0; mi < m_dwMeshNum; mi++)   // メッシュ数だけ繰り返し
		{
			...
			if (m_nMeshType == 1)     // スタティックメッシュ
			{
				...
				for (int i = 0; i < tnum; i++)
				{
					...
					m_pMeshArray[mi].m_pMaterialDiffuse = m_pMaterialDiffuseArray[n];   // マテリアル・ディフューズ色   
					m_pMeshArray[mi].m_pMaterialSpecular = m_pMaterialSpecularArray[n]; // マテリアル・スペキュラー色   
					p += sizeof(int);
				}
...
BOOL  CFbxMesh::SetTexture(const DWORD& ti, const TCHAR* TexName)
{
	...
	// マテリアルカラーのクリヤー
	m_pMaterialDiffuseArray[ti] = VECTOR4(0, 0, 0, 0);     
	m_pMaterialSpecularArray[ti] = VECTOR4(0, 0, 0, 0);    

	//テクスチャー作成
	m_pTextureArray[ti] = NULL;
	m_pTextureArray[ti] = m_pFbxMeshCtrl->SetTextureList(TexName);                     
	...
	m_pTextureNormalArray[ti] = m_pFbxMeshCtrl->SetTextureList(fullname);                 
	...
	m_pTextureHeightArray[ti] = m_pFbxMeshCtrl->SetTextureList(fullname);                   
	...
	m_pTextureSpecularArray[ti] = m_pFbxMeshCtrl->SetTextureList(fullname);                   
...
BOOL CFbxMesh::LoadAnimation(const TCHAR* FName, const ROOTANIMTYPE& RAType)
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		// ------------------------------------------------------------------------------------------------
		// アニメーションに関するデータ
		memcpy_s(&m_Animation[m_allAnimationCount].startFrame, sizeof(int), p, sizeof(int));       // 開始フレームを得る
		p += sizeof(int);
		memcpy_s(&m_Animation[m_allAnimationCount].endFrame, sizeof(int), p, sizeof(int));         // 終了フレームを得る
		p += sizeof(int);

		// ルートボーンに関するデータ
		memcpy_s(&m_Animation[m_allAnimationCount].RootBoneMesh, sizeof(int), p, sizeof(int));       // ルートボーンメッシュ番号を得る   
		p += sizeof(int);
		memcpy_s(&m_Animation[m_allAnimationCount].RootBone, sizeof(int), p, sizeof(int));           // ルートボーン番号を得る            
		p += sizeof(int);
		...
		for (mi = 0; mi < m_dwMeshNum; mi++)   // メッシュ数だけ繰り返し
		{
			...
			for (bi = 0; bi < m_pMeshArray[mi].m_NumBones; bi++)
			{
				// バインドポーズの取り出し
				// （バインドポーズは、逆行列となっている）
				memcpy_s(&m_pMeshArray[mi].m_BoneArray[m_allAnimationCount][bi].bindPose, sizeof(MATRIX4X4), p, sizeof(MATRIX4X4));  // バインドポーズ行列の転送
				p += sizeof(MATRIX4X4);

				// フレームポーズ数の取り出し
				memcpy_s(&m_pMeshArray[mi].m_BoneArray[m_allAnimationCount][bi].framePoseNum, sizeof(int), p, sizeof(int));
				p += sizeof(int);
				m_pMeshArray[mi].m_BoneArray[m_allAnimationCount][bi].framePose = new MATRIX4X4[m_pMeshArray[mi].m_BoneArray[m_allAnimationCount][bi].framePoseNum];  // フレームポーズ毎のボーン配列を生成する

				// フレームポーズの取り出し
				for (n = 0; n < m_pMeshArray[mi].m_BoneArray[m_allAnimationCount][bi].framePoseNum; n++)
				{
					memcpy_s(&m_pMeshArray[mi].m_BoneArray[m_allAnimationCount][bi].framePose[n], sizeof(MATRIX4X4), p, sizeof(MATRIX4X4));  // フレームポーズ行列の転送
					p += sizeof(MATRIX4X4);
				}
			}
		}
		...
		m_allAnimationCount++;    // アニメーションカウントを進める

		// -------------------------------------------------------------------------------
		// ルートボーンアニメーションとシェーダーマトリックスの作成               
		MakeRootAnimAndShaderMatrix(m_allAnimationCount-1, RAType);
			...
			for (int bi = 0; bi < m_pMeshArray[mi].m_NumBones; bi++)  // 各ボーン毎の処理
			{
				...
				if (RAType == eRootAnimNone)
				{
					...
					mFramePose = m_pMeshArray[mi].m_BoneArray[animNum][bi].framePose[n] *
									XMMatrixInverse(NULL, m_RootBoneArray[animNum].framePose[n]);      // 各フレームポーズ行列をルートボーン行列との差分行列にする          
					mFramePose = m_pMeshArray[mi].m_BoneArray[animNum][bi].bindPose * mFramePose;      // バインドボーズの逆行列とフレームポーズを掛け合わせ、バインドボーズからの差分の行列を作成する       
				}
...
MATRIX4X4 CFbxMesh::GetFrameMatrices(ANIMATION_STATUS& animStatus, const DWORD& nBone, const DWORD& nMesh)
{
	...
	mBoneWorld = m_pMeshArray[nMesh].m_BoneArray[animStatus.animNum][nBone].framePose[animFrameW]; 
	...
	mBoneWorld = XMMatrixScaling(1, 1, -1) * mBoneWorld;   
...
void CFbxMesh::RenderToon(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		RenderToonStatic(mWorld, mView, mProj, vLight, vEye);
	}
	else  if (m_nMeshType == 2) {
		if (m_allAnimationCount <= 0)
		{
		...
		ANIMATION_STATUS animStatus;
		RenderToonSkin(animStatus, mWorld, mView, mProj, vLight, vEye);
	}
}


void CFbxMesh::RenderAfter(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye ,const AFTERSTATUS& Aftersta)
{
	if (m_nMeshType == 1)
	{
		RenderAfterStatic(mWorld, mView, mProj, vLight, vEye,Aftersta);
	}
	else  if (m_nMeshType == 2) {
		if (m_allAnimationCount <= 0)
		{
		...
		ANIMATION_STATUS animStatus;
		RenderAfterSkin(animStatus, mWorld, mView, mProj, vLight, vEye,Aftersta);
	}
}
...
//
//------------------------------------------------------------------------
void CFbxMesh::Render(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		RenderStatic(mWorld, mView, mProj, vLight, vEye);
...
void CFbxMesh::RenderDisplace(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		RenderDisplaceStatic(mWorld, mView, mProj, vLight, vEye);
...
void CFbxMesh::RenderToon(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	if (m_nMeshType == 1)
	{
		RenderToonStatic(mWorld, mView, mProj, vLight, vEye);
...
void CFbxMesh::RenderAfter(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye,const AFTERSTATUS& Aftersta)
{
	if (m_nMeshType == 1)
	{
		RenderAfterStatic(mWorld, mView, mProj, vLight, vEye,Aftersta);
...
void CFbxMesh::RenderStatic(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	...
	for (DWORD mi = 0; mi < m_dwMeshNum; mi++)  
	{
		...
		// ディフューズテクスチャがないときのみシェーダーのコンスタントバッファーにマテリアルカラーを渡す        
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderSkin(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	// アニメーションのループ　+　前フレームに再生していたアニメーション番号と違うアニメーション番号の場合最初から再生させる
	if (animStatus.animNum != animStatus.animNumOld) animStatus.animFrame = 0;
	animStatus.endFrame = m_Animation[animStatus.animNum].endFrame - m_Animation[animStatus.animNum].startFrame;

	// メッシュの描画
	Draw(animStatus.animNum, animStatus.animFrame, mWorld, mView, mProj, vLight, vEye);

	//	アニメーションの更新
	if (animStatus.playAnim) {
		animStatus.animFrame++;
		//if (animStatus.animFrame >= m_Animation[animStatus.animNum].endFrame - m_Animation[animStatus.animNum].startFrame)
		if (animStatus.animFrame > m_Animation[animStatus.animNum].endFrame - m_Animation[animStatus.animNum].startFrame)
		{
			animStatus.animFrame = 0;
			animStatus.endFlag = true;
			animStatus.animLock = false;
		}
		else {
			animStatus.endFlag = false;
		}
		animStatus.animNumOld = animStatus.animNum; // 前のアニメーション番号の更新
	}
}
...
//
//------------------------------------------------------------------------
void CFbxMesh::Draw(const int& animNum, const int& frame, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderDisplaceSkin(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{

	//m_pD3D->m_pDeviceContext->RSSetState(m_pD3D->m_pRStateLW);  // ワイヤフレーム
	...
	// メッシュの描画
	DrawDisplace(animStatus.animNum, animStatus.animFrame, mWorld, mView, mProj, vLight, vEye);
	...
	//m_pD3D->m_pDeviceContext->RSSetState(m_pD3D->m_pRStateL);  // 通常描画
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderToonStatic(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	// Pass1・Pass2共通処理  --------------------------------------------------------------------------------------
	// トゥーンテクスチャをシェーダーに渡す
	SetShaderTexture(_T("PS"), 4, &m_pD3D->m_pToonTexture);
	...
	if (SUCCEEDED(m_pD3D->m_pDeviceContext->Map(m_pShader->m_pConstantBufferWVLED, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
	{
		...
		// 各種情報を渡す。ここではテクスチャ有りを渡す。(全てテクスチャ有りのため使っていない)
		cb.vDrawInfo = VECTOR4(1, 0, 0, 0);
	...
	// Pass1  --------------------------------------------------------------------------------------
	//	（トゥーンインク（輪郭線）を描画する）
	//	・メッシュを法線方向に輪郭線の太さ分拡大し、裏面のみを描画する

	// ラスタライザステート　裏面を描画する
	m_pD3D->m_pDeviceContext->RSSetState(m_pD3D->m_pRStateLB);

	//使用するシェーダーの登録	
	m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxStaticMeshToon_VS_TOON1, NULL, 0);
	m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxStaticMeshToon_PS_TOON1, NULL, 0);
	...
	//	・トゥーンテクスチャを使用する。
	//	・メッシュ面の明るさを、トゥーンテクスチャのＵ座標に変換して明るさを平坦にする。

	// ラスタライザステート　左回りを表とする
	m_pD3D->m_pDeviceContext->RSSetState(m_pD3D->m_pRStateL);

	//使用するシェーダーの登録	
	m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxStaticMeshToon_VS_TOON2, NULL, 0);
	m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxStaticMeshToon_PS_TOON2, NULL, 0);
	...
	for (DWORD mi = 0; mi < m_dwMeshNum; mi++)  
	{
		...
		// テクスチャ（ディフューズ、ノーマル）をピクセルシェーダーに渡す。スロット番号は0～1
		SetShaderTexture(_T("PS"), 0, &m_pMeshArray[i].m_pTexture);             // ディフューズテクスチャ
		SetShaderTexture(_T("PS"), 1, &m_pMeshArray[i].m_pTextureNormal);       // ノーマルテクスチャ
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderToonSkin(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	...
	// メッシュの描画
	DrawToon(animStatus.animNum, animStatus.animFrame, mWorld, mView, mProj, vLight, vEye);
...
//------------------------------------------------------------------------
//
//	スキンメッシュを画面にトゥーン描画する処理
//
//	Pass1（トゥーンインク（輪郭線）を描画する）
//	・メッシュを法線方向に輪郭線の太さ分拡大し、裏面のみを描画する
//
//	Pass2（トゥーンペイント（陰影を平坦にして描画する））
//	・トゥーンテクスチャを使用する。
//	・メッシュ面の明るさを、トゥーンテクスチャのＵ座標に変換して明るさを平坦にする。
//
// 引数
...
//
//------------------------------------------------------------------------
void CFbxMesh::DrawToon(const int& animNum, const int& frame, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
	// Pass1・Pass2共通処理  --------------------------------------------------------------------------------------
	// トゥーンテクスチャをシェーダーに渡す
	SetShaderTexture(_T("PS"), 4, &m_pD3D->m_pToonTexture);
	...
	// Pass1  --------------------------------------------------------------------------------------
	//	（トゥーンインク（輪郭線）を描画する）
	//	・メッシュを法線方向に輪郭線の太さ分拡大し、裏面のみを描画する(背面法)

	// ラスタライザステート　裏面を描画する
	m_pD3D->m_pDeviceContext->RSSetState(m_pD3D->m_pRStateLB);

	//使用するシェーダーの登録	
	m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxSkinMeshToon_VS_TOON1, NULL, 0);
	m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxSkinMeshToon_PS_TOON1, NULL, 0);
	...
	//使用するシェーダーの登録	
	m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxSkinMeshToon_VS_TOON2, NULL, 0);
	m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxSkinMeshToon_PS_TOON2, NULL, 0);
...
//      const VECTOR3& vLight                 光源ベクトル
//      const VECTOR3& vEye                   視点ベクトル
//		const AFTERSTATUS& Aftersta			  残像ステータス
//
//	戻り値 HRESULT
//		S_OK	= 正常
//		E_FAIL	= 異常
//
//------------------------------------------------------------------------
void CFbxMesh::RenderAfterStatic(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye,const AFTERSTATUS& Aftersta)
{
	// Pass1・Pass2共通処理  --------------------------------------------------------------------------------------
	// 残像テクスチャをシェーダーに渡す
	SetShaderTexture(_T("PS"), 4, &m_pD3D->m_pToonTexture);
	...
	//使用するシェーダーの登録	
	m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxStaticMeshAfter_VS_AFTER1, NULL, 0);
	m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxStaticMeshAfter_PS_AFTER1, NULL, 0);
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderAfterSkin(ANIMATION_STATUS& animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye,const AFTERSTATUS& Aftersta)
{
	...
	// メッシュの描画
	DrawAfter(animStatus.animNum, animStatus.animFrame, mWorld, mView, mProj, vLight, vEye,Aftersta);
...
//
//------------------------------------------------------------------------
void CFbxMesh::DrawAfter(const int& animNum, const int& frame, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye,const AFTERSTATUS& Aftersta)
{
	for (int Num = 0; Num < 4; Num++)
	{
		...
		{
			//ワールド行列を渡す
			cb.mW = XMMatrixTranspose(Aftersta.mWorldOld[Num]);

			//ワールド、カメラ、射影行列を渡す
			cb.mWVP = XMMatrixTranspose(Aftersta.mWorldOld[Num] * mView * mProj);
		...
		//使用するシェーダーの登録	
		m_pD3D->m_pDeviceContext->VSSetShader(m_pShader->m_pFbxSkinMeshAfter_VS_AFTER1, NULL, 0);
		m_pD3D->m_pDeviceContext->PSSetShader(m_pShader->m_pFbxSkinMeshAfter_PS_AFTER1, NULL, 0);
		...
		for (DWORD mi = 0; mi < m_dwMeshNum; mi++)   
		{
			...
			if (SUCCEEDED(m_pD3D->m_pDeviceContext->Map(
				...
				&pData)))                        // データの書き込み先ポインタ
			{
				DWORD No = Aftersta.SaveFrameNo[Num];
				DWORD Fream = Aftersta.SaveFrame[Num];
				// このメッシュのシェーダーボーン配列のうち、指定されたアニメーション番号（animNum）、フレーム番号(frame)のボーン行列配列を先頭からボーン数だけ転送する
				memcpy_s(pData.pData, sizeof(MATRIX4X4)*m_pMeshArray[i].m_NumBones, &m_pMeshArray[i].m_pBoneShader[No][Fream].shaderFramePose, sizeof(MATRIX4X4)*m_pMeshArray[i].m_NumBones);		// cbBonesの書き込み 全ボーンのフレームポーズ行列を渡す
				m_pD3D->m_pDeviceContext->Unmap(m_pShader->m_pConstantBufferBone2, 0);
			}
...
//
//------------------------------------------------------------------------
void  CFbxMesh::SetShaderTexture(TCHAR ProfileName[], int SlotNo, ID3D11ShaderResourceView** ppTexture)
{
...
//
//------------------------------------------------------------------------
void  CFbxMesh::SetShaderMatColor(TCHAR ProfileName[], int SlotNo, int i )
{
	int prof = _tcscmp(_T("PS"), ProfileName);  // "PS"のみのときは0, "DSPS"のときは0以外
