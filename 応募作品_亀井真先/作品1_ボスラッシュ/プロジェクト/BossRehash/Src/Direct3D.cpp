...
HRESULT CDirect3D::InitD3D(HWND hWnd, DWORD dwWidth, DWORD dwHeight)
{
	...
	rdc.FrontCounterClockwise = FALSE;		// 三角形の頂点がレンダーターゲット上で右回りならば三角形は前向きと見なされる
	m_pDevice->CreateRasterizerState(&rdc, &m_pRStateR);

	rdc.FrontCounterClockwise = TRUE;		// 三角形の頂点がレンダーターゲット上で左回りならば三角形は前向きと見なされる
	m_pDevice->CreateRasterizerState(&rdc, &m_pRStateL);
	...
	rdc.FrontCounterClockwise = TRUE;		// 三角形の頂点がレンダーターゲット上で左回りならば三角形は前向きと見なされる
	rdc.FillMode = D3D11_FILL_WIREFRAME;	// ワイヤーフレーム表示
	m_pDevice->CreateRasterizerState(&rdc, &m_pRStateLW);

	m_pDeviceContext->RSSetState(m_pRStateR);  // 最初は右回りは前向きと設定する
	m_nRStateFlag = 0;  // 最初は右回りは前
...
//  ラスタライザステートの設定
//
//	int nFlag	: 0:右回りは前向き 1:左回りは前向き
//
//  戻り値　　　なし
//------------------------------------------------------------------------
void CDirect3D::SetRState(int nFlag)
{
	// ラスタライザステートの設定
	if (m_nRStateFlag != nFlag)
	{
		switch (nFlag)
		{
		case 0:
			m_pDeviceContext->RSSetState(m_pRStateR);  // 右回りは前向きと設定する
			break;
		case 1:
			m_pDeviceContext->RSSetState(m_pRStateL);  // 左回りは前向きと設定する
			break;
		}
		m_nRStateFlag = nFlag;
	}
...
HRESULT CDirect3D::InitSampler()
{
	...
	memcpy_s(SamDesc.BorderColor, sizeof(SamDesc.BorderColor), &VECTOR4(1.0f, 1.0f, 1.0f, 0.0f), sizeof(VECTOR4)); // Addressにて_BORDERが指定されている場合に使用する境界色 (範囲: 0.0~1.0)
