...
{
	// ステージ０　泉のステージ
	MapRead( _T("Data/Script/Map201.txt"), m_pMapArray[0]);

	// ステージ１　お城のステージ
	MapRead( _T("Data/Script/Map302.txt"), m_pMapArray[1]);

	// ステージ２　お城のステージ２
	MapRead( _T("Data/Script/Map301.txt"), m_pMapArray[2]);

	// ステージ3　泉のステージ
	MapRead( _T("Data/Script/Map303.txt"), m_pMapArray[3]);

	// ステージ4　泉のステージ2
	MapRead(_T("Data/Script/Map304.txt"), m_pMapArray[4]);

	//ステージ5　最後のステージ
	MapRead(_T("Data/Script/Map305.txt"), m_pMapArray[5]);
...
void CMapProc::MoveMap()
{
	...
	if (m_pGMain->m_nDoorStop[3] != 1) {
		if (m_pMapArray[0] && m_pGMain->m_nPcpositionx <= 1700 && m_pGMain->m_nPcpositiony >= 700) {
			m_pGMain->m_nBoomStop = 0;
			m_pGMain->m_nHeelNum = m_pGMain->m_nHeelMaxNum + 1;
			SetMap(3);	// 次のマップ３
			m_pGMain->m_pEnmProc->SetNonActive();	// 敵を全て消す。敵の発生フラグをリセットする
	...
	if (m_pMapArray[0] &&  m_pGMain->m_nPcpositiony < 500) {
		m_pGMain->m_nBoomStop = 0;
		m_pGMain->m_nHeelNum = m_pGMain->m_nHeelMaxNum + 1;
		SetMap(5);	// 次のマップ４
		m_pGMain->m_pEnmProc->SetNonActive();	// 敵を全て消す。敵の発生フラグをリセットする
