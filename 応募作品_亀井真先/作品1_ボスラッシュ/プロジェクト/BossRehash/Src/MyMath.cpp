//-----------------------------------------------------------------------------
//
//  Direct3Dを利用するための汎用３Ｄライブラリ          ver 2.8        2020.2.15
// 
//	各種　数学系のライブラリ
//
//
...
VECTOR3 GetRotateVector3(const MATRIX4X4& mWorld)
{
	...
	if (fabs((double)mWorld._23 - 1.0) < threshold)  // Mat(2,3) sin(x)=1のとき
	{
		angle.x = XM_PI / 2;
		angle.y = 0;
		angle.z = atan2((double)mWorld._12, (double)mWorld._11);
	}
	else if (fabs((double)mWorld._23 + 1.0) < threshold)  // Mat(2,3) sin(x)=-1のとき
	{
		angle.x = -XM_PI / 2;
		angle.y = 0;
		angle.z = atan2((double)mWorld._12, (double)mWorld._11);
		...
		angle.x = asin((double)mWorld._23);
		angle.y = atan2(-(double)mWorld._13, (double)mWorld._33);
		angle.z = atan2(-(double)mWorld._21, (double)mWorld._22);
	}
